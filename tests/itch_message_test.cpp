...
TEST_CASE("decode_order_delete decodes a hand-crafted Order Delete message", "[itch_message]") {
  ...
  std::vector<std::uint8_t> bytes{0x44, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
TEST_CASE("decode_add_order decodes a hand-crafted Add Order message", "[itch_message]") {
  ...
  std::vector<std::uint8_t> bytes{0x41, 0x00, 0x2a, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                  0x00, 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
                                  0x08, 0x42, 0x00, 0x00, 0x00, 0x64, 0x00, 0x00, 0x00,
...
TEST_CASE("decode_order_executed decodes a hand-crafted Order Executed message", "[itch_message]") {
  // Order Executed layout (engine/docs/itch-5.0-message-formats.md):
  // offset 0: Message Type "E", offset 1-2: Stock Locate, offset 3-4: Tracking Number,
  // offset 5-10: Timestamp, offset 11-18: Order Reference Number, offset 19-22: Executed
  // Shares, offset 23-30: Match Number (unused, not decoded)
  std::vector<std::uint8_t> bytes{0x45, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                  0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x00, 0x00, 0x00,
                                  0x4b, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};

  engine::OrderExecuted result = engine::decode_order_executed(bytes);

  REQUIRE(result.order_reference_number == 72623859790382856);
  REQUIRE(result.quantity == 75);
}

TEST_CASE(
    "decode_order_executed_with_price decodes a hand-crafted Order Executed With Price message",
    "[itch_message]") {
  // Order Executed With Price layout (engine/docs/itch-5.0-message-formats.md):
  // offset 0: Message Type "C", offset 1-2: Stock Locate, offset 3-4: Tracking Number,
  // offset 5-10: Timestamp, offset 11-18: Order Reference Number, offset 19-22: Executed
  // Shares, offset 23-30: Match Number (unused), offset 31: Printable (unused),
  // offset 32-35: Execution Price
  std::vector<std::uint8_t> bytes{0x43, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                  0x00, 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
                                  0x08, 0x00, 0x00, 0x00, 0x4b, 0x00, 0x00, 0x00, 0x00,
                                  0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x0f, 0x42, 0x40};

  engine::OrderExecutedWithPrice result = engine::decode_order_executed_with_price(bytes);
...
TEST_CASE("decode_message dispatches an Order Executed message correctly", "[itch_message]") {
  // same hand-crafted Order Executed bytes as above
  std::vector<std::uint8_t> bytes{0x45, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                  0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x00, 0x00, 0x00,
                                  0x4b, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
  ...
  REQUIRE(std::holds_alternative<engine::OrderExecuted>(result));
}

TEST_CASE("decode_message dispatches an Order Executed With Price message correctly",
          "[itch_message]") {
  // same hand-crafted Order Executed With Price bytes as above
  std::vector<std::uint8_t> bytes{0x43, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                  0x00, 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
                                  0x08, 0x00, 0x00, 0x00, 0x4b, 0x00, 0x00, 0x00, 0x00,
                                  0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x0f, 0x42, 0x40};
